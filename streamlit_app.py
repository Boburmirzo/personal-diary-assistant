...
import os
import re
from diary_assistant import (
    PersonalDiaryAssistant,
...
def show_login_form():
    ...
    with col1:
        ...
        username = st.text_input(
            ...
            help="This will create a personalized database for your diary entries",
        )
        ...
        api_key = st.text_input(
            ...
            type="password",
            placeholder="sk-...",
            help="Your OpenAI API key is needed for AI-powered insights. Get one at https://platform.openai.com/api-keys",
        )

        # Validation and submit
        submit_button = st.button("🚀 Start My Diary Journey", type="primary")
        ...
        if submit_button:
            ...
            try:
                ...
                st.session_state.assistant = PersonalDiaryAssistant(
                    database_path=database_path, openai_api_key=api_key.strip()
                )
                st.session_state.authenticated = True
                st.success(f"✅ Welcome {username}! Your personal diary is ready.")
                st.rerun()

            except Exception as e:
                st.error(f"❌ Error initializing your diary: {str(e)}")
                return False

    with col2:
        st.markdown("#### 🌟 What you'll get:")
        st.markdown("""
        - **🧠 Advanced Memory**: AI that remembers and learns from your entries
        - **📊 Pattern Analysis**: Understand your mood, productivity, and habits
        - **🎯 Personalized Advice**: Get recommendations tailored to your patterns
        - **📈 Visual Analytics**: Beautiful charts and insights
        - **🔒 Privacy First**: Your data stays secure and private
        - **🌱 Personal Growth**: Track progress and achieve your goals
        """)

        st.markdown("#### 🔑 About Your API Key:")
        st.markdown("""
        - Your API key is used to power the AI features
        - It's stored securely in your session only
        - We don't store or share your API key
        - Get your free key at [OpenAI Platform](https://platform.openai.com/api-keys)
        """)

        st.markdown("#### 📱 Multi-Device Access:")
        st.markdown("""
        - Use the same name to access your diary from any device
        - Your entries are saved in a personal database
